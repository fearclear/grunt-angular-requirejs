define(['app', 'storage'], function (app, storage) {
    return app.controller('LoginCtrl', ['$scope', '$rootScope', 'serverService', '$cookies', '$location', '$timeout', 'baseService',
                ...
                if(isFirst){
                    ...
                    serverService.login($scope.user).then(function (data) {
                        ...
                        if (data.userId != null) {
                            ...
                            if ($scope.user.isRem) {
                                localStorage.setItem('__user__', $scope.user.name);
                            } else {
                                localStorage.removeItem('__user__');
                            }
                            if (data.changePassword == 'true') {
                                // $scope.isConfirmChange = true;
                                $location.path('/changePassword').replace()
                            } else {
                                $scope.$emit('getParams', {})
                                $location.path('/transactionKanban').replace();
                                $scope.$emit('changeWsLink', {token: data.userId})
                            }
                        } else {
